...
- Notes:
  ...
  - Pagination is explicit through `X-WP-Total`, `X-WP-TotalPages`, and `Link`. The complete site exposed 130 jobs / 85 companies and required two pages at `per_page=100`; the engineering category exposed 26 jobs on one page. The category-scoped response was approximately 110 KB over the wire and 1.5 MB decoded. A normal adapter needs no individual detail requests; resolve the category once per scan, fetch sequential pages, cap at two job pages, and cap the complete scan at three HTTP requests.
  - Access constraints were benign for ordinary HTTP: the homepage, robots file, feed, REST discovery, taxonomies, and bounded post queries all returned HTTP 200 without redirects or rate limits. Cloudflare and a bot-management cookie were present but did not challenge the descriptive `job-tracker-bot/1.0` user agent. `robots.txt` allows general crawling except `/wp-admin/` and explicitly allows `/wp-admin/admin-ajax.php`; no prohibited path is needed. The audit used 15 sequential GETs and approximately 0.9 MB total wire payload, with no aggressive crawl.
  ...
  - **Discovery validation reliability follow-up (2026-08-28).** GitHub Actions evidence showed that discovery and per-candidate live validation completed, but the workflow skipped every commit after the full 199-board diagnostic began failing on the retired EGYM and Pitch Personio tenants. Both tenant roots and XML feeds now permanently redirect to `https://personio.com`, so those two boards are disabled while remaining in the registry for history. The scheduled workflow keeps the full-catalog diagnostic visible but non-blocking; candidate promotion remains gated by `validate_candidate()` and the existing live-job threshold before `companies.toml` can change. The required v2 gate also exposed five unrelated recency failures after fixed August 7/11 fixtures aged beyond 14 days; their setup now uses current timestamps or an explicit 365-day limit where the test owns deduplication rather than recency. No adapter, scheduler, filter, database, dependency, concurrency, or runtime-memory behavior changed.
  - Follow-up verification: focused discovery tests passed 6/6; the five previously date-expired cases passed across seven parameterized checks; `python -m pytest tests/v2 -q` and `python -m pytest -q --timeout=30` each passed 552 tests with one existing third-party `audioop` warning. The `job-bot:discovery-validation-fix` image built successfully. Its live validator reported 197 healthy / zero failed boards, and the representative Personio dry scan was healthy with 1,357 raw / six accepted, HTTP peak 4/4, and no persistence or notification send. An isolated loopback-only service with ephemeral data/log files became ready with 13 scheduled sources, returned compact backward-compatible `/health`, and used 55.02 MiB / 512 MiB before the first scheduled refresh. The ephemeral service was removed. No historical diagnostic was needed for this focused catalog/workflow/test-fixture correction; live provider availability remains external and the GitHub workflow result must still be confirmed after push.
